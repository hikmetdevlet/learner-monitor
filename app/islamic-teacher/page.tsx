...
const BOTTOM_TABS = [
  { key: 'dashboard',  label: 'Home',      icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="m3 9 9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/><polyline points="9 22 9 12 15 12 15 22"/></svg> },
  { key: 'attendance', label: 'Attendance', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M17 21v-2a4 4 0 0 0-4-4H5a4 4 0 0 0-4 4v2"/><circle cx="9" cy="7" r="4"/><polyline points="16 11 18 13 22 9"/></svg> },
  { key: 'islamic',    label: 'Topics',     icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg> },
]
...
export default function IslamicTeacherDashboard() {
  ...
  async function loadMyClasses(tId: string) {
    const { data } = await supabase.from('islamic_teacher_classes').select('*, classes(id, name, class_type)').eq('teacher_id', tId)
  ...
  async function loadDashboardStats(classList: any[]) {
    ...
    for (let i = 6; i >= 0; i--) {
      ...
      days.push({ day: d.toLocaleDateString('en-US', { weekday: 'short' }), total, present, pct: total > 0 ? Math.round((present / total) * 100) : 0, hasData: total > 0 })
    }
    setWeeklyAtt(days)

    const classIds = classList.map((c: any) => c.id)
    const { data: subjData } = await supabase.from('curriculum_subjects').select('*, curriculum_topics(id)').in('class_id', classIds).eq('is_active', true)
    const tStats: any[] = []
    for (const subj of (subjData || [])) {
      ...
      const { data: trackedTopics } = await supabase.from('curriculum_topics').select('id').in('id', trackedTopicIds).eq('track_per_learner', true).eq('is_active', true)
      if (!trackedTopics || trackedTopics.length === 0) continue
      const topicIds = trackedTopics.map((t: any) => t.id)
      const { data: progressData } = await supabase.from('learner_topic_progress').select('*').in('topic_id', topicIds).eq('completed', true)
      const totalPossible = topicIds.length * allStats.length
      const completed = progressData?.length || 0
      tStats.push({ subject: subj, topicCount: topicIds.length, completed, totalPossible, pct: totalPossible > 0 ? Math.round((completed / totalPossible) * 100) : 0 })
    }
  ...
  async function loadAttendanceForDate(sessionId: string, date: string, learnerList: any[]) {
    const { data: attData } = await supabase.from('attendance').select('*').eq('timetable_id', sessionId).eq('attendance_date', date)
    const attMap: Record<string, string> = {}
    const notesMap: Record<string, string> = {}
    learnerList.forEach(l => { attMap[l.id] = 'absent' })
    attData?.forEach((a: any) => { attMap[a.learner_id] = a.status; if (a.note) notesMap[a.learner_id] = a.note })
    setAttendance(attMap); setAttNotes(notesMap)
  }
  ...
  async function saveAttendance() {
    ...
    for (const learner of learners) {
      await supabase.from('attendance').upsert({
        timetable_id: selectedSession.id, learner_id: learner.id, attendance_date: attDate,
        status: attendance[learner.id] || 'absent', excused: attendance[learner.id] === 'excused',
  ...
  async function selectClass(cls: any) {
    setSelectedClass(cls); setSelectedSubject(null); setTopics([]); setProgress({})
    const { data } = await supabase.from('learner_classes').select('*, learners(id, full_name)').eq('class_id', cls.id)
    setIslamicLearners(data?.map((lc: any) => lc.learners).filter(Boolean) || [])
    const { data: subjData } = await supabase.from('curriculum_subjects').select('*').eq('class_id', cls.id).eq('is_active', true).order('order_num')
    setSubjects(subjData || [])
  }

  async function selectSubject(subject: any) {
    setSelectedSubject(subject); setSavedProgress(false)
    const { data: topicData } = await supabase.from('curriculum_topics').select('*').eq('subject_id', subject.id).eq('is_active', true).eq('track_per_learner', true).order('order_num')
    setTopics(topicData || [])
    if (islamicLearners.length > 0 && topicData && topicData.length > 0) {
      const { data: progressData } = await supabase.from('learner_topic_progress').select('*')
        .in('learner_id', islamicLearners.map((l: any) => l.id))
        .in('topic_id', topicData.map((t: any) => t.id))
      const map: Record<string, Record<string, boolean>> = {}
      progressData?.forEach((p: any) => { if (!map[p.learner_id]) map[p.learner_id] = {}; map[p.learner_id][p.topic_id] = p.completed })
      setProgress(map)
    }
  }
    ...
    for (const learner of islamicLearners) {
      for (const topic of topics) {
        ...
        await supabase.from('learner_topic_progress').upsert({
          learner_id: learner.id, topic_id: topic.id, completed,
    ...
    <main style={{ minHeight: '100vh', background: '#F8F7F4', fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .brand-icon { width:30px; height:30px; background:#15803D; border-radius:8px; display:flex; align-items:center; justify-content:center; }
        ...
        .bnav-btn {
          ...
          color:#bbb; border-radius:10px; transition:all 0.15s; position:relative;
        }
        .bnav-btn.active { color:#15803D; }
        .bnav-btn.active svg { stroke: #15803D; }
        .bnav-btn.active::before {
          content:''; position:absolute; top:0; left:50%; transform:translateX(-50%);
          width:20px; height:2.5px; background:#15803D; border-radius:0 0 3px 3px;
        }
        ...
        /* ── Session / class / subject buttons ── */
        .nav-btn { background:#fff; border:1px solid #EFEFED; border-radius:12px; padding:16px 20px; margin-bottom:10px; cursor:pointer; transition:all 0.15s; display:flex; align-items:center; justify-content:space-between; width:100%; text-align:left; font-family:'DM Sans',sans-serif; }
        .nav-btn:hover { border-color:#BBF7D0; background:#F0FDF4; }

        /* ── Attendance marking ── */
        .back-btn { font-size:13px; color:#AAA; background:none; border:none; cursor:pointer; font-family:'DM Sans',sans-serif; display:flex; align-items:center; gap:4px; margin-bottom:6px; padding:0; }
        .back-btn:hover { color:#555; }
        .att-header { display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:20px; flex-wrap:wrap; gap:12px; }
        ...
        .date-input { height:36px; border:1px solid #EFEFED; border-radius:9px; padding:0 12px; font-size:13px; font-family:'DM Sans',sans-serif; color:#1A1A1A; background:#fff; outline:none; }
        .date-input:focus { border-color:#15803D; }
        .save-btn { background:#15803D; color:white; border:none; border-radius:9px; padding:9px 20px; font-size:13px; font-weight:500; cursor:pointer; font-family:'DM Sans',sans-serif; white-space:nowrap; }
        ...
        .learner-card { background:#fff; border:1px solid #EFEFED; border-radius:12px; padding:14px 16px; margin-bottom:8px; }
        ...
        .status-btn { padding:5px 10px; border-radius:8px; border:2px solid transparent; cursor:pointer; font-size:11px; font-weight:500; font-family:'DM Sans',sans-serif; transition:all 0.15s; }
        ...
        /* ── Progress table ── */
        .progress-header { display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:20px; flex-wrap:wrap; gap:10px; }
        ...
        @media (max-width: 768px) {
          ...
          .learner-att-row { padding: 10px 14px; }
          .card-head { padding: 12px 14px; }
          .att-header { flex-direction: column; }
          .att-controls { width: 100%; justify-content: space-between; }
          .att-summary { grid-template-columns: repeat(4, 1fr); gap: 6px; }
          .att-sum-n { font-size: 16px; }
          .att-sum-l { font-size: 9px; }
          .status-btns { gap: 4px; }
          .status-btn { padding: 4px 8px; font-size: 10px; }
      ...
      <div className="tab-bar">
        {BOTTOM_TABS.map(tab => (
          <button key={tab.key} className={`tab-btn ${activeTab === tab.key ? 'active' : ''}`} onClick={() => switchTab(tab.key)}>
            {tab.icon} {tab.label === 'Home' ? 'Dashboard' : tab.label === 'Topics' ? 'Topic Tracking' : tab.label}
          </button>
        ))}
      </div>
          ...
          <div>
            ...
            <div className="card">
              <div className="card-head">
                ...
                <span style={{ fontSize: 11, color: '#AAA' }}>{stats.length} learners</span>
              </div>
              {stats.length === 0 ? (
                ...
                stats.map((s: any) => (
                  <div key={s.learner.id} className="learner-att-row">
                    <div style={{ minWidth: 120 }}>
                      <div style={{ fontSize: 13, fontWeight: 500, color: '#1A1A1A' }}>{s.learner.full_name}</div>
                      <div style={{ fontSize: 11, color: '#AAA' }}>{s.cls.name}</div>
                    </div>
                    <div className="progress-bar">
                      <div className="progress-fill" style={{ width: `${s.pct}%`, background: s.pct >= 70 ? '#22C55E' : '#EF4444' }} />
                    </div>
                    <span style={{ fontSize: 13, fontWeight: 500, color: s.pct >= 70 ? '#15803D' : '#DC2626', minWidth: 36, textAlign: 'right' }}>{s.pct}%</span>
                  </div>
                ))
              )}
            </div>
            ...
            {!selectedSession ? (
              <div>
                ...
                {todaySessions.length === 0 ? (
                  ...
                  todaySessions.map(s => (
                    <div key={s.id} className="nav-btn" onClick={() => selectSession(s)}>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 500, color: '#1A1A1A' }}>{s.name}</div>
                        <div style={{ fontSize: 12, color: '#AAA', marginTop: 2 }}>{s.classes?.name} · {s.start_time?.slice(0,5)} – {s.end_time?.slice(0,5)}</div>
                      </div>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#15803D" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                    </div>
                  ))
                )}
              </div>
              ...
              <div>
                <div className="att-header">
                  <div>
                    <button className="back-btn" onClick={() => { setSelectedSession(null); setSaved(false) }}>
                ...
                <div className="mark-all-row">
                  ...
                  {STATUS_OPTIONS.map(s => (
                    <button key={s.key} className="mark-all-btn" onClick={() => markAll(s.key)} style={{ background: s.light, color: s.text }}>{s.label}</button>
                  ))}
                </div>
                ...
                {learners.map(l => {
                  ...
                  return (
                    <div key={l.id} className="learner-card" style={{ borderColor: cfg?.light || '#EFEFED' }}>
                      <div className="learner-card-top">
                        <div className="learner-card-name">
                          <div className="status-dot" style={{ background: cfg?.bg || '#EF4444' }} />
                          {l.full_name}
                        </div>
          ...
          <div>
            {!selectedClass ? (
              <div>
                ...
                {myClasses.length === 0 ? (
                  <div className="empty-state"><p className="empty-title">No classes assigned</p><p className="empty-sub">Contact admin to assign classes</p></div>
                ) : (
                  myClasses.map(cls => (
                    <div key={cls.id} className="nav-btn" onClick={() => selectClass(cls)}>
                      <div>
                        <div style={{ fontSize: 14, fontWeight: 500, color: '#1A1A1A' }}>{cls.name}</div>
                        <div style={{ fontSize: 12, color: '#AAA', marginTop: 2 }}>Islamic class</div>
                      </div>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#15803D" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                    </div>
                  ))
              ...
              <div>
                ...
                {subjects.length === 0 ? (
                  <div className="empty-state"><p className="empty-title">No subjects in curriculum</p><p className="empty-sub">Admin needs to add subjects in Curriculum Management</p></div>
                ) : (
                  subjects.map(s => (
                    <div key={s.id} className="nav-btn" onClick={() => selectSubject(s)}>
                      <div style={{ fontSize: 14, fontWeight: 500, color: '#1A1A1A' }}>{s.name}</div>
                      <svg width="16" height="16" viewBox="0 0 24 24" fill="none" stroke="#15803D" strokeWidth="2"><polyline points="9 18 15 12 9 6"/></svg>
                    </div>
                  ))
              ...
              <div>
                <div className="progress-header">
                  ...
                  <button className="save-btn" onClick={saveProgress} disabled={savingProgress}>
                    {savingProgress ? 'Saving...' : savedProgress ? '✓ Saved' : 'Save progress'}
                  </button>
                </div>

                {topics.length === 0 ? (
                  <div className="empty-state"><p className="empty-title">No tracked topics</p><p className="empty-sub">Admin needs to enable "Track per learner" on topics</p></div>
                ) : islamicLearners.length === 0 ? (
                  <div className="empty-state"><p className="empty-title">No learners in this class</p></div>
                ) : (
      ...
      <nav className="bottom-nav">
        <div className="bottom-nav-inner">
          {BOTTOM_TABS.map(tab => (
            <button key={tab.key} className={`bnav-btn ${activeTab === tab.key ? 'active' : ''}`} onClick={() => switchTab(tab.key)}>
              {tab.icon}
              {tab.label}
            </button>
          ))}
