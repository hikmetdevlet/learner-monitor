...
export const MAT_COLORS: Record<string, { bg: string; c: string }> = {
...
export function weekBounds(offsetWeeks = 0) {
  ...
  return {
    ws: s, we: e,
    wsStr: s.toISOString().split('T')[0],
    weStr: e.toISOString().split('T')[0],
  }
}

export function topicStatus(t: { planned_start?: string; planned_end?: string }): 'this-week' | 'overdue' | 'upcoming' | 'no-date' {
  if (!t.planned_start) return 'no-date'
  const { ws, we } = weekBounds()
  const s = new Date(t.planned_start)
  const e = t.planned_end ? new Date(t.planned_end) : s
  if (e < ws) return 'overdue'
  if (s <= we && e >= ws) return 'this-week'
  return 'upcoming'
}

export function dedup<T extends { id: string }>(list: T[]): T[] {
...
export function attendanceStreak(attData: Array<{ attendance_date: string; status: string }>): number {
  ...
  for (let i = 0; i < 30; i++) {
    ...
    const rec = attData.find(a => a.attendance_date === ds)
