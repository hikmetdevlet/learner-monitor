...
export default function TeacherDashboard() {
  ...
  const [eTitle, setETitle] = useState(''); const [eDate, setEDate] = useState(''); const [eClass, setEClass] = useState(''); const [eDesc, setEDesc] = useState(''); const [eSaving, setESaving] = useState(false)
  ...
  async function loadDashboard(t: any) {
    ...
    const [{ data: sess }, { data: cls }, { data: exms }, { data: hwData }] = await Promise.all([
      t.is_head_teacher
    ...
    setTodaySessions(sess || []); setUpcomingExams(exms || []); setRecentHW(hwData || [])
    const classes = t.is_head_teacher ? (cls || []) : [...new Map((cls || []).map((s: any) => [s.classes?.id, s.classes]).filter(([k]: any) => k)).values()]
    setMyClasses(classes); setAllClasses(classes); if (classes.length > 0) setEClass(classes[0].id)
    ...
    for (const c of classes) {
      const { data: lc } = await supabase.from('learner_classes').select('learner_id').eq('class_id', c.id)
  ...
  async function loadBehaviour(t: any) {
    ...
    const ll = (lc || []).map((x: any) => ({ id: x.learners?.id, full_name: x.learners?.full_name, class_id: x.class_id, class_name: x.classes?.name })).filter(l => l.id)
    setAllLearners(ll)
    const ids = ll.map(l => l.id)
  ...
  async function saveBeh() {
    ...
    setBehSaving(true)
    await supabase.from(behModal === 'incident' ? 'behaviour_incidents' : 'behaviour_praise').insert({ learner_id: behLearner, teacher_id: teacher.id, type: behType, note: behNote.trim() || null })
  ...
  // Curriculum derived
  const grades = useMemo(() => { const m = new Map<string, any>(); subjects.forEach(s => { if (s.classes) m.set(s.classes.id, s.classes) }); return [...m.values()] }, [subjects])
  const gradeSubjects = useMemo(() => activeGrade ? subjects.filter(s => s.class_id === activeGrade) : [], [subjects, activeGrade])
  ...
  function filteredTopics() {
    ...
    if (currFilter === 'week') {
      const { ws, we } = weekBounds()
      t = t.filter(x => { if (!x.planned_start) return false; const s = new Date(x.planned_start), e = x.planned_end ? new Date(x.planned_end) : s; return s <= we && e >= ws })
  ...
  async function selSess(s: any) {
    ...
    a?.forEach((x: any) => { am[x.learner_id] = x.status }); h?.forEach((x: any) => { hm[x.learner_id] = x.submitted }); n?.forEach((x: any) => { nm[x.learner_id] = x.content })
    ...
    <main style={{ minHeight: '100vh', background: '#F8F7F4', fontFamily: "'DM Sans',sans-serif" }}>
      ...
      {fbModal && (
        <div className="mov" onClick={e => { if (e.target === e.currentTarget) setFbModal(null) }}>
          <div className="mo">
            ...
            <textarea className="mta" placeholder="Bu derste neler öğrenildi, hangi konular anlaşılmadı..." value={fbNote} onChange={e => setFbNote(e.target.value)} rows={3} />
            <div className="macts">
      ...
      {behModal && (
        <div className="mov" onClick={e => { if (e.target === e.currentTarget) setBehModal(null) }}>
          <div className="mo">
            <div className="mtit">{behModal === 'incident' ? '⚠️ Disiplin Kaydı' : '⭐ Takdir Kaydı'}</div>
            <div className="msub">{behModal === 'incident' ? 'Olumsuz davranış kaydı' : 'Olumlu davranış kaydı'}</div>
            ...
            <div className="mlbl" style={{ marginBottom: 5 }}>Açıklama (isteğe bağlı)</div>
      ...
      <div className="topbar">
        <div className="brand">
          <div className="brand-icon"><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="white" strokeWidth="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg></div>
      ...
      <div className="tab-bar">
        {BOTTOM_TABS.map(t => <button key={t.key} className={`tab-btn ${activeTab === t.key ? 'active' : ''}`} onClick={() => switchTab(t.key)}>{t.icon} {t.label}</button>)}
      </div>
      ...
      <div className="wrap">
        ...
        {activeTab === 'dashboard' && (
          <div>
            <div style={{ marginBottom: 16 }}><div className="h1">Günaydın, {teacher?.display_name?.split(' ')[0] || teacher?.full_name?.split(' ')[0]}</div><div className="sub">{dateFmt}</div></div>
            <div className="s3">
              <div className="scard"><div className="sicon" style={{ background: '#EFF6FF' }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#1D4ED8" strokeWidth="2"><path d="M3 9l9-7 9 7v11a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2z"/></svg></div><div className="sn">{myClasses.length}</div><div className="sl">Sınıf</div></div>
              <div className="scard"><div className="sicon" style={{ background: '#ECFEFF' }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#0E7490" strokeWidth="2"><path d="M2 3h6a4 4 0 0 1 4 4v14a3 3 0 0 0-3-3H2z"/><path d="M22 3h-6a4 4 0 0 0-4 4v14a3 3 0 0 1 3-3h7z"/></svg></div><div className="sn" style={{ color: thisWk > 0 ? '#0E7490' : undefined }}>{thisWk}</div><div className="sl">Bu Hafta</div></div>
              <div className="scard"><div className="sicon" style={{ background: '#FFF7ED' }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#C2410C" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg></div><div className="sn">{upcomingExams.length}</div><div className="sl">Sınav</div></div>
            </div>
            {overdue > 0 && <div style={{ background: '#FEF2F2', border: '1px solid #FCA5A5', borderRadius: 9, padding: '9px 13px', marginBottom: 12, display: 'flex', alignItems: 'center', gap: 7 }}><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="#DC2626" strokeWidth="2"><circle cx="12" cy="12" r="10"/><line x1="12" y1="8" x2="12" y2="12"/><line x1="12" y1="16" x2="12.01" y2="16"/></svg><span style={{ fontSize: 12, color: '#DC2626', fontWeight: 600 }}>{overdue} gecikmeli konu</span><button className="go" style={{ marginLeft: 'auto', background: '#FEF2F2', color: '#DC2626', borderColor: '#FCA5A5' }} onClick={() => switchTab('curriculum')}>İncele →</button></div>}
            {thisWk > 0 && <div className="card"><div className="ch"><span className="ct">Bu Haftanın Konuları</span><button className="go" onClick={() => switchTab('curriculum')}>Tümü →</button></div>{topics.filter(t => topicStatus(t) === 'this-week').slice(0, 4).map(t => { const p = progress.find(x => x.topic_id === t.id); return <div key={t.id} className="lr"><div><div className="rn" style={{ display: 'flex', alignItems: 'center', gap: 4 }}>{p?.is_completed && <svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="#16A34A" strokeWidth="3"><polyline points="20 6 9 17 4 12"/></svg>}{t.title}</div><div className="rs">{t.curriculum_subjects?.name} · {t.curriculum_subjects?.classes?.name}</div></div><span className="bdg" style={{ background: p?.is_completed ? '#DCFCE7' : '#E0F2FE', color: p?.is_completed ? '#16A34A' : '#0284C7' }}>{p?.is_completed ? '✓' : 'Bu Hafta'}</span></div> })}</div>}
            <div className="card"><div className="ch"><span className="ct">Bugün — {todayName}</span><button className="go" onClick={() => switchTab('attendance')}>Yoklama →</button></div>{todaySessions.length === 0 ? <div className="empty">Bugün ders yok</div> : todaySessions.map(s => <div key={s.id} className="lr"><div><div className="rn">{s.name}</div><div className="rs">{s.classes?.name} · {s.start_time?.slice(0,5)}–{s.end_time?.slice(0,5)}</div></div><span className="bdg" style={{ background: '#EFF6FF', color: '#1D4ED8' }}>{s.start_time?.slice(0,5)}</span></div>)}</div>
            <div className="card"><div className="ch"><span className="ct">Yoklama Oranı</span></div>{attStats.map(s => <div key={s.cls.id} className="lr"><div><div className="rn">{s.cls.name}</div><div className="rs">{s.n} öğrenci</div></div><div className="pbar"><div className="btrack"><div className="bfill" style={{ width: `${s.pct}%`, background: s.pct >= 70 ? '#22C55E' : '#EF4444' }} /></div><span className="pt" style={{ color: s.pct >= 70 ? '#15803D' : '#DC2626' }}>{s.pct}%</span></div></div>)}</div>
            {incidents.slice(0,3).length > 0 && <div className="card"><div className="ch"><span className="ct">Son Disiplin</span><button className="go" onClick={() => switchTab('behaviour')}>Tümü →</button></div>{incidents.slice(0,3).map(i => { const cfg = INCIDENT_TYPES.find(t => t.key === i.type) || INCIDENT_TYPES[6]; return <div key={i.id} className="lr"><div><div className="rn">{i.learners?.full_name}</div><div className="rs">{cfg.label} · {fmt(i.created_at)}</div></div><span className="bdg" style={{ background: cfg.bg, color: cfg.c }}>{cfg.label}</span></div> })}</div>}
          </div>
        )}
        ...
        {activeTab === 'attendance' && (
          <div>
            {!selSession ? (
              <div>
                ...
                {todaySessions.length === 0 ? <div className="card"><div className="empty">Bugün ders yok</div></div> : todaySessions.map(s => (
                  <button key={s.id} className="sess-btn" onClick={() => selSess(s)}>
                    <div><div style={{ fontSize: 13, fontWeight: 600, color: '#1A1A1A' }}>{s.name}</div><div style={{ fontSize: 11, color: '#AAA', marginTop: 1 }}>{s.classes?.name} · {s.start_time?.slice(0,5)}–{s.end_time?.slice(0,5)}</div></div>
              ...
              <div>
                <div className="atthr">
                  <div><button className="bk" onClick={() => { setSelSession(null); setSaved(false) }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg> Geri</button><div className="h1">{selSession.name}</div><div className="sub">{selSession.classes?.name} · {selSession.start_time?.slice(0,5)}–{selSession.end_time?.slice(0,5)}</div></div>
                ...
                {learners.map(l => {
                  ...
                  return (
                    <div key={l.id} className="lcard" style={{ borderColor: status === 'present' ? '#BBF7D0' : status === 'late' ? '#FDE68A' : '#EFEFED' }}>
                      <div className="ltop">
                        <div className="lname"><div className="sdot" style={{ background: status === 'present' ? '#22C55E' : status === 'late' ? '#EAB308' : '#E5E5E5' }} />{l.full_name}</div>
                        <div className="sbtns">
                      ...
                      <div className="brow2">
                        <label className="hwck"><input type="checkbox" checked={hw[l.id] || false} onChange={e => { setHw(p => ({ ...p, [l.id]: e.target.checked })); setSaved(false) }} /> Ödev teslim</label>
        ...
        {activeTab === 'curriculum' && (
          <div>
            {currView === 'list' ? (
              <div>
                <div style={{ marginBottom: 14 }}><div className="h1">Müfredat Takibi</div><div className="sub">Konu planı ve ilerleme durumu</div></div>
                ...
                {ft.length === 0 ? (
                  <div style={{ background: '#fff', border: '1px solid #EFEFED', borderRadius: 11, padding: 36, textAlign: 'center' }}>
                    ...
                    <div style={{ fontSize: 12, fontWeight: 600, color: '#555', marginBottom: 2 }}>{currFilter === 'week' ? 'Bu hafta için planlanmış konu yok' : 'Konu bulunamadı'}</div>
                    <div style={{ fontSize: 10, color: '#AAA' }}>{currFilter === 'week' ? '"Tümü"ne geçerek tüm konuları görün' : 'Admin müfredat sayfasından tarih ekleyebilir'}</div>
                  </div>
                ) : ft.map(t => {
                  const p = progress.find(x => x.topic_id === t.id); const isd = p?.is_completed; const st = isd ? 'done' : topicStatus(t)
                  return (
                      ...
                      <div className="tcin">
                        ...
                        <div className="tci">
                          ...
                          <div className="tcmeta">
                            ...
                            {t.planned_start && <span style={{ display: 'flex', alignItems: 'center', gap: 2 }}><svg width="8" height="8" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg>{fmt(t.planned_start)}{t.planned_end ? ` → ${fmt(t.planned_end)}` : ''}</span>}
                          </div>
                        </div>
              ...
              <div>
                <button className="bk" onClick={() => { setCurrView('list'); setSelTopic(null) }}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="15 18 9 12 15 6"/></svg> Müfredada Dön</button>
                ...
                {selTopic.planned_start && (
                  <div className="dban">
                    ...
                    {(() => { const s = topicStatus(selTopic); const p = progress.find(x => x.topic_id === selTopic.id); if (p?.is_completed) return <span className="sc sc-dn" style={{ marginLeft: 4 }}>Done</span>; if (s === 'this-week') return <span className="sc sc-tw" style={{ marginLeft: 4 }}>Bu Hafta</span>; if (s === 'overdue') return <span className="sc sc-ov" style={{ marginLeft: 4 }}>Gecikti</span>; return null })()}
                  </div>
                )}
                <div style={{ marginBottom: 14 }}>
                  ...
                  {materials.length === 0 ? <div style={{ background: '#FAFAF8', border: '1px solid #F0F0EE', borderRadius: 8, padding: 16, textAlign: 'center', color: '#CCC', fontSize: 11 }}>Materyal eklenmemiş</div> : materials.map(m => { const cfg = MAT_COLORS[m.type] || MAT_COLORS.link; return <div key={m.id} className="mitem"><div className="mico" style={{ background: cfg.bg, color: cfg.c }}>{MAT_ICONS[m.type]}</div><div style={{ flex: 1, minWidth: 0 }}><div className="mname">{m.title}</div>{m.url && <div className="mlink">{m.url}</div>}{m.content && <div style={{ fontSize: 10, color: '#888', marginTop: 1 }}>{m.content}</div>}</div>{m.url && <a href={m.url} target="_blank" rel="noreferrer" className="olink">Aç ↗</a>}</div> })}
                </div>
                {(() => {
                  const p = progress.find(x => x.topic_id === selTopic.id); const isc = p?.is_completed; const ucfg = UNDERSTAND[p?.understanding as keyof typeof UNDERSTAND]
                  return <div>{isc && <div className="fbox"><div className="fbtit"><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>Tamamlandı — {fmt(p.taught_date)}</div>{ucfg && <span style={{ fontSize: 10, fontWeight: 700, padding: '2px 8px', borderRadius: 5, background: ucfg.bg, color: ucfg.c }}>{ucfg.e} {ucfg.label}</span>}{p.feedback_note && <div style={{ fontSize: 11, color: '#555', marginTop: 6, lineHeight: 1.5 }}>{p.feedback_note}</div>}</div>}<div className="dacts">{!isc ? <button className="act" style={{ background: '#15803D', color: '#fff' }} onClick={() => openFb(selTopic)}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5"><polyline points="20 6 9 17 4 12"/></svg>Tamamlandı İşaretle</button> : <><button className="act" style={{ background: '#7E22CE', color: '#fff' }} onClick={() => openFb(selTopic)}>Güncelle</button><button className="act" style={{ background: '#F5F5F3', color: '#666', border: '1px solid #EFEFED' }} onClick={() => unmark(selTopic.id)}>Geri Al</button></>}</div></div>
                })()}
              </div>
            )}
          </div>
        ...
        {activeTab === 'behaviour' && (
          <div>
            ...
            <div className="bseg">
              <button className={`bseg-btn ${behTab === 'incidents' ? 'on' : ''}`} onClick={() => setBehTab('incidents')}>⚠️ Disiplin ({filtInc.length})</button>
            ...
            {behTab === 'incidents' && (
              filtInc.length === 0
                ? <div style={{ background: '#fff', border: '1px solid #EFEFED', borderRadius: 11, padding: 36, textAlign: 'center' }}><div style={{ fontSize: 26, marginBottom: 7 }}>✅</div><div style={{ fontSize: 12, fontWeight: 600, color: '#555' }}>Disiplin kaydı yok</div></div>
                : filtInc.map(i => { const cfg = INCIDENT_TYPES.find(t => t.key === i.type) || INCIDENT_TYPES[6]; const lrn = allLearners.find(l => l.id === i.learner_id); return <div key={i.id} className="brec" style={{ borderLeft: `3px solid ${cfg.c}` }}><div className="brh"><div><div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}><div className="blrn">{i.learners?.full_name}</div>{lrn?.class_name && <span className="bctag">{lrn.class_name}</span>}</div><span className="btyp" style={{ background: cfg.bg, color: cfg.c }}>{cfg.label}</span></div><button className="delb" onClick={() => delRec('behaviour_incidents', i.id)}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></button></div>{i.note && <div className="bnote">{i.note}</div>}<div className="btime">{fmtDT(i.created_at)}</div></div> })
            )}
            {behTab === 'praise' && (
              filtPr.length === 0
                ? <div style={{ background: '#fff', border: '1px solid #EFEFED', borderRadius: 11, padding: 36, textAlign: 'center' }}><div style={{ fontSize: 26, marginBottom: 7 }}>⭐</div><div style={{ fontSize: 12, fontWeight: 600, color: '#555' }}>Takdir kaydı yok</div></div>
                : filtPr.map(i => { const cfg = PRAISE_TYPES.find(t => t.key === i.type) || PRAISE_TYPES[5]; const lrn = allLearners.find(l => l.id === i.learner_id); return <div key={i.id} className="brec" style={{ borderLeft: `3px solid ${cfg.c}` }}><div className="brh"><div><div style={{ display: 'flex', alignItems: 'center', gap: 6, marginBottom: 3 }}><div className="blrn">{i.learners?.full_name}</div>{lrn?.class_name && <span className="bctag">{lrn.class_name}</span>}</div><span className="btyp" style={{ background: cfg.bg, color: cfg.c }}>{cfg.label}</span></div><button className="delb" onClick={() => delRec('behaviour_praise', i.id)}><svg width="11" height="11" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></button></div>{i.note && <div className="bnote">{i.note}</div>}<div className="btime">{fmtDT(i.created_at)}</div></div> })
            )}
          </div>
        )}
        ...
        {activeTab === 'exams' && (
          <div>
            <div className="hrow">
              <div><div className="h1">Sınavlar</div><div className="sub">Sınav takvimi</div></div>
              <button className="abtn" onClick={() => setShowExamForm(!showExamForm)}>{showExamForm ? 'İptal' : <><svg width="9" height="9" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg>Sınav Ekle</>}</button>
            </div>
            {showExamForm && (
              <div className="fcard">
                <div className="fgrid">
                  <div className="fg"><label className="flb">Başlık *</label><input className="fin" value={eTitle} onChange={e => setETitle(e.target.value)} placeholder="ör. Matematik 1. Dönem" /></div>
                  <div className="fg"><label className="flb">Tarih *</label><input className="fin" type="date" value={eDate} onChange={e => setEDate(e.target.value)} /></div>
                  <div className="fg"><label className="flb">Sınıf *</label><select className="fse" value={eClass} onChange={e => setEClass(e.target.value)}>{allClasses.map((c: any) => <option key={c.id} value={c.id}>{c.name}</option>)}</select></div>
                  <div className="fg"><label className="flb">Açıklama</label><input className="fin" value={eDesc} onChange={e => setEDesc(e.target.value)} placeholder="Kapsam, format..." /></div>
                </div>
                <div className="fas2">
            ...
            {exams.filter(e => e.exam_date < todayStr).length > 0 && <div className="card" style={{ marginBottom: 10 }}><div className="ch"><span className="ct">Geçmiş</span></div>{exams.filter(e => e.exam_date < todayStr).map(e => <div key={e.id} className="lr"><div><div className="rn">{e.title}</div><div className="rs">{e.classes?.name} · {e.exam_date}</div></div><div style={{ display: 'flex', gap: 4, alignItems: 'center' }}><span className="bdg" style={{ background: '#F5F5F3', color: '#888' }}>Bitti</span><button className="db2" onClick={() => { supabase.from('exams').update({ is_active: false }).eq('id', e.id); setExams(p => p.filter(x => x.id !== e.id)) }}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></button></div></div>)}</div>}
            <div className="card"><div className="ch"><span className="ct">Yaklaşan Sınavlar</span></div>{exams.filter(e => e.exam_date >= todayStr).length === 0 ? <div className="empty">Yaklaşan sınav yok</div> : exams.filter(e => e.exam_date >= todayStr).map(e => { const d = Math.ceil((new Date(e.exam_date).getTime() - new Date().getTime()) / 86400000); return <div key={e.id} className="lr"><div><div className="rn">{e.title}</div><div className="rs">{e.classes?.name} · {e.exam_date}{e.description && ` · ${e.description}`}</div></div><div style={{ display: 'flex', gap: 5, alignItems: 'center' }}><span className="ecd" style={{ color: d <= 3 ? '#DC2626' : d <= 7 ? '#A16207' : '#15803D' }}>{d === 0 ? 'Bugün!' : d === 1 ? 'Yarın!' : `${d}g`}</span><button className="db2" onClick={() => { supabase.from('exams').update({ is_active: false }).eq('id', e.id); setExams(p => p.filter(x => x.id !== e.id)) }}><svg width="10" height="10" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2"><polyline points="3 6 5 6 21 6"/><path d="M19 6l-1 14H6L5 6"/></svg></button></div></div> })}</div>
          </div>
        )}
      ...
      <nav className="bnav">
        <div className="bnav-inner">
          {BOTTOM_TABS.map(t => <button key={t.key} className={`bnav-btn ${activeTab === t.key ? 'active' : ''}`} onClick={() => switchTab(t.key)}>{t.icon}{t.label}</button>)}
        </div>
      </nav>
    </main>
  )
