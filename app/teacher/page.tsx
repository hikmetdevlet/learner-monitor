...
const BOTTOM_TABS = [
  ...
  { key: 'attendance', label: 'Attendance', icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><polyline points="9 11 12 14 22 4"/><path d="M21 12v7a2 2 0 0 1-2 2H5a2 2 0 0 1-2-2V5a2 2 0 0 1 2-2h11"/></svg> },
  { key: 'homework',   label: 'Homework',   icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><path d="M14 2H6a2 2 0 0 0-2 2v16a2 2 0 0 0 2 2h12a2 2 0 0 0 2-2V8z"/><polyline points="14 2 14 8 20 8"/></svg> },
  { key: 'exams',      label: 'Exams',      icon: <svg width="18" height="18" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="1.8" strokeLinecap="round" strokeLinejoin="round"><rect x="3" y="4" width="18" height="18" rx="2"/><line x1="16" y1="2" x2="16" y2="6"/><line x1="8" y1="2" x2="8" y2="6"/><line x1="3" y1="10" x2="21" y2="10"/></svg> },
]
    ...
    <main style={{ minHeight: '100vh', background: '#F8F7F4', fontFamily: "'DM Sans', sans-serif" }}>
      <style>{`
        ...
        *, *::before, *::after { box-sizing: border-box; }
        ...
        .brand-icon { width:30px; height:30px; background:#1D4ED8; border-radius:8px; display:flex; align-items:center; justify-content:center; }
        ...
        .bnav-btn.active::before {
          ...
          width:20px; height:2.5px; background:#1D4ED8; border-radius:0 0 3px 3px;
        }
        ...
        /* ── Attendance marking ── */
        .back-btn { font-size:13px; color:#AAA; background:none; border:none; cursor:pointer; font-family:'DM Sans',sans-serif; display:flex; align-items:center; gap:4px; padding:0; margin-bottom:8px; }
        .back-btn:hover { color:#555; }
        .att-header { display:flex; align-items:flex-start; justify-content:space-between; margin-bottom:20px; gap:12px; }
        .att-title { font-family:'DM Serif Display',serif; font-size:20px; color:#1A1A1A; }
        .att-meta { font-size:12px; color:#AAA; margin-top:2px; }
        .save-btn { background:#1D4ED8; color:white; border:none; border-radius:9px; padding:9px 20px; font-size:13px; font-weight:500; cursor:pointer; font-family:'DM Sans',sans-serif; white-space:nowrap; flex-shrink:0; }
        .save-btn:hover { background:#1E40AF; }
        .save-btn:disabled { opacity:0.5; }
        .learner-card { background:#fff; border:1.5px solid #EFEFED; border-radius:11px; padding:13px 16px; margin-bottom:8px; transition:border-color 0.15s; }
        .learner-top { display:flex; align-items:center; justify-content:space-between; margin-bottom:10px; flex-wrap:wrap; gap:8px; }
        ...
        .bottom-row { display:flex; gap:12px; margin-top:8px; align-items:center; flex-wrap:wrap; }
        .hw-check { display:flex; align-items:center; gap:6px; font-size:12px; color:#666; cursor:pointer; white-space:nowrap; }
        .note-input { flex:1; min-width:120px; height:32px; border:1px solid #F0F0EE; border-radius:8px; padding:0 10px; font-size:12px; font-family:'DM Sans',sans-serif; color:#555; background:#FAFAF8; outline:none; }
        .note-input:focus { border-color:#1D4ED8; background:#fff; }
        .note-input::placeholder { color:#CCC; }

        /* ── Exam form ── */
        .add-btn { display:flex; align-items:center; gap:6px; background:#1A1A1A; color:white; border:none; border-radius:8px; padding:8px 14px; font-size:12px; font-weight:500; cursor:pointer; font-family:'DM Sans',sans-serif; }
        ...
        .exam-countdown { font-size:13px; font-weight:600; }
        .delete-btn { background:none; border:none; cursor:pointer; color:#CCC; padding:4px; }
        .delete-btn:hover { color:#DC2626; }
        .go-btn { font-size:11px; background:#EFF6FF; color:#1D4ED8; border:1px solid #BFDBFE; border-radius:7px; padding:4px 10px; cursor:pointer; font-family:'DM Sans',sans-serif; font-weight:500; white-space:nowrap; }
        ...
        @media (max-width: 768px) {
          ...
          .status-btns { gap: 4px; }
          .status-btn { padding: 5px 9px; font-size: 10px; }
          .form-grid { grid-template-columns: 1fr; }
          .pct-bar { min-width: 80px; }
      ...
      <div className="tab-bar">
        {BOTTOM_TABS.map(tab => (
          <button key={tab.key} className={`tab-btn ${activeTab === tab.key ? 'active' : ''}`} onClick={() => switchTab(tab.key)}>
            {tab.icon} {tab.label === 'Home' ? 'Dashboard' : tab.label}
          </button>
        ))}
      </div>
          ...
          <div>
            ...
            <div className="card">
              <div className="card-head">
                <span className="card-title">Today — {todayName}</span>
                <button className="go-btn" onClick={() => switchTab('attendance')}>Mark attendance →</button>
              </div>
              {todaySessions.length === 0 ? <div className="empty">No sessions today</div> : (
                todaySessions.map(s => (
            ...
            <div className="card">
              ...
              {attendanceStats.length === 0 ? <div className="empty">No data yet</div> : (
                attendanceStats.map(s => (
                  <div key={s.cls.id} className="list-row">
                    ...
                    <div className="pct-bar">
                      <div className="bar-track"><div className="bar-fill" style={{ width: `${s.pct}%`, background: s.pct >= 70 ? '#22C55E' : '#EF4444' }} /></div>
            ...
            {upcomingExams.length > 0 && (
              <div className="card">
                <div className="card-head">
                  ...
                  <button className="go-btn" onClick={() => switchTab('exams')}>Manage →</button>
                </div>
                {upcomingExams.map(e => {
            ...
            <div className="card">
              <div className="card-head">
                ...
                <button className="go-btn" onClick={() => switchTab('homework')}>All →</button>
              </div>
              {recentHomework.length === 0 ? <div className="empty">No homework assigned yet</div> : (
                  ...
                  return (
                    <div key={hw.id} className="list-row">
                      <div>
                        ...
                        <div className="row-sub">{isHead && hw.users && `${hw.users.display_name || hw.users.full_name} · `}{hw.classes?.name} · Due: {hw.due_date}</div>
                      </div>
                      <div className="pct-bar">
                        <div className="bar-track"><div className="bar-fill" style={{ width: `${pct}%`, background: pct >= 70 ? '#22C55E' : '#EF4444' }} /></div>
                  ...
                  todaySessions.map(s => (
                    <button key={s.id} className="session-btn" onClick={() => selectSession(s)}>
                      <div>
                        ...
                        <div style={{ fontSize: 12, color: '#AAA', marginTop: 2 }}>{s.classes?.name} · {s.start_time?.slice(0,5)} – {s.end_time?.slice(0,5)}{isHead && s.users && ` · ${s.users.display_name || s.users.full_name}`}</div>
                      </div>
                    ...
                    <div key={l.id} className="learner-card" style={{ borderColor: status === 'present' ? '#BBF7D0' : status === 'late' ? '#FDE68A' : '#EFEFED' }}>
                        ...
                        <div className="status-btns">
                          {(['present', 'late', 'absent'] as const).map(s => (
                            <button key={s} className="status-btn"
                              ...
                              style={{ background: status === s ? STATUS_COLORS[s].bg : '#F8F8F6', color: status === s ? STATUS_COLORS[s].text : '#AAA', borderColor: status === s ? STATUS_COLORS[s].bg : 'transparent' }}>
                      ...
                      <div className="bottom-row">
                        <label className="hw-check">
                          <input type="checkbox" checked={homework[l.id] || false} onChange={e => { setHomework(prev => ({ ...prev, [l.id]: e.target.checked })); setSaved(false) }} />
                          HW submitted
                        </label>
                        <input className="note-input" value={notes[l.id] || ''} placeholder="Add note..."
        ...
        {activeTab === 'homework' && (
          <div>
            <div className="page-header-row">
              ...
              <button className="add-btn" onClick={() => router.push('/teacher/homework')}>
            ...
            <div className="card">
              ...
              {recentHomework.length === 0 ? <div className="empty">No homework yet</div> : (
                recentHomework.map(hw => {
                  const subs = hw.homework_submissions || []
                  const submitted = subs.filter((s: any) => s.status === 'submitted_on_time' || s.status === 'submitted_late').length
                  const pct = subs.length > 0 ? Math.round((submitted / subs.length) * 100) : 0
          ...
          <div>
            <div className="page-header-row">
              <div>
                ...
                <p className="page-date">Upcoming exam dates</p>
              </div>
              <button className="add-btn" onClick={() => setShowExamForm(!showExamForm)}>
                {showExamForm ? 'Cancel' : <><svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" strokeWidth="2.5" strokeLinecap="round"><line x1="12" y1="5" x2="12" y2="19"/><line x1="5" y1="12" x2="19" y2="12"/></svg> Add exam</>}
              </button>
            </div>
            ...
            {showExamForm && (
              <div className="form-card">
                ...
                <div className="form-actions">
                  ...
                  <button style={{ background: '#1A1A1A', color: 'white', border: 'none', borderRadius: 8, padding: '8px 16px', fontSize: 13, fontWeight: 500, cursor: 'pointer', fontFamily: "'DM Sans',sans-serif" }} onClick={addExam} disabled={examSaving}>{examSaving ? 'Saving...' : 'Add exam'}</button>
                </div>
              </div>
            )}
                ...
                {exams.filter(e => e.exam_date < todayStr).map(e => (
                  <div key={e.id} className="list-row">
                    ...
                    <div style={{ display: 'flex', gap: 6, alignItems: 'center' }}>
                      ...
                      <button className="delete-btn" onClick={() => deleteExam(e.id)}>
